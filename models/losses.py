...
class Lovasz_ce_loss(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.lambda_lovasz = self.config.TRAIN.LOSS.get('lambda_lovasz', 0.1)
        if config.TRAIN['CLASS_WEIGHTS'] != []:
            seg_num_per_class = config.TRAIN['CLASS_WEIGHTS']
    ...
    def forward(self, data_dict):
        loss_main_ce = self.ce_loss(data_dict['logits'], data_dict['edge_label'].long())
        loss_main_lovasz = self.lovasz_loss(F.softmax(data_dict['logits'], dim=1), data_dict['edge_label'].long())
