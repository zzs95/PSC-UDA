...
from data.kidney_dataloader import get_public_set, get_TESTSET
...
def build_dataloader(cfg, mode='train', domain='source', start_iteration=0, max_iteration=None,
    ...
    if dataset_cfg.TYPE == 'KITS':
        # source domain
        dataset = get_public_set(split, domain, dataset_kwargs)
    elif 'CHAOS' in dataset_cfg.TYPE:
        dataset = get_public_set(split, domain, dataset_kwargs)
    elif 'AMOS' in dataset_cfg.TYPE:
        dataset = get_public_set(split, domain, dataset_kwargs)
    ...
    collate_fn = get_collate_scn(point_num=dataset_cfg['point_num'], crop_pselab=crop_pselab, pselab_path=pselab_path)
