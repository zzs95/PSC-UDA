    ...
    with torch.no_grad():
        for iteration, data_batch_trg in enumerate(dataloader):
            ...
            if model_img != None:
                ...
                pred_label_voxel_img = data_batch_trg['img_seg_logits'].argmax(1).cpu()
            ...
            for batch_ind in range(len(seg_label)):
                ...
                # turn space back to 1x1x1
                affine_matrix = data_batch_trg['edge'].affine[batch_ind]
                pts_boundary_1 = np.concatenate([pts_boundary_tansformed, np.array([[0], [0]])], axis=1)
            ...
            if model_img != None:
                val_metric_logger.update(loss_edge_seg_fuse_f_trg=data_batch_trg['loss_edge_seg_fuse_f_trg'],)
                val_metric_logger.update(loss_edge_seg_img_trg=data_batch_trg['loss_edge_seg_img_trg'],)
