...
def get_collate_scn(point_num, crop_pselab=False, pselab_path=None):
    sample_mode = 'rand'
...
def read_pselab_edge(pselab_edge_path):
    ...
    if crop_pselab:
        ...
        for i_batch in range(batch_size):
            ...
            if os.path.exists(boundary_npy_path):
                ...
                if input_dict['domain'][i_batch] == 'source':
                    # pselab_edge = input_dict['pselab_edge'][i_batch, 0] # load gt in 160,320,320 size
                    pselab_edge_path = input_dict['pselab_edge'].meta['filename_or_obj'][i_batch] # load gt in 1x1x1 spacing
                    pselab_edge = read_pselab_edge(pselab_edge_path)

                elif input_dict['domain'][i_batch] == 'target':
                    pselab_edge_path = os.path.join(pselab_path, 'val_output', edge_name)
                    pselab_edge = read_pselab_edge(pselab_edge_path) # load last point cloud prediction in 1x1x1 spacing
                pselab_edge_tmp = deepcopy(pselab_edge)
                pselab_nonz = (pselab_edge_tmp >= 2).nonzero() # get kidney point cloud (src: gt, trg: prediction)
                if len(pselab_nonz) < 20:
                    ...
                    edge_bottom_dist = np.abs(np.floor(((kidney_bottom - edge_bottom_last) * 0.1)).astype(int))
                    edge_top_dist = np.abs(np.floor(((edge_top_last - kidney_top) * 0.1)).astype(int))
                    edge_bottom_cur = edge_bottom_last + edge_bottom_dist
                    edge_top_cur = edge_top_last - edge_top_dist
                #  scale image in space 1x1x1 to fixsize 160,320,320
