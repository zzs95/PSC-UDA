# https://raw.githubusercontent.com/ChenHoy/pyreg/538ec75f9762020f2185ab785f781dc67ee4ffc8/lib/point_cloud_class.py
import numpy
...
class PointCloud(object):
    ...
    def __init__(self, points=None, normals=None, color=None, curvature=None, bounding_box=None, fpfh=None,
                 key_points=None):
    ...
    def read_pcd(self, file_path):
        ...
        if file_extension == ".yml" or file_extension == ".yaml":
            ...
            with open(file_path, "rb") as f:
                ...
                self.points.T[0], self.points.T[1], self.points.T[2] = plydata.elements[0].data["x"][:], \
                ...
                if len(properties) > 3:
                    ...
                    self.normals.T[0], self.normals.T[1], self.normals.T[2] = plydata.elements[0].data["nx"][:], \
                                                                              plydata.elements[0].data["ny"][:], \
                                                                              plydata.elements[0].data["nz"][:]
                else:
                    pass
    ...
    def write_pcd(self, file_format, filepath, only_save_points=False):
        ...
        if file_format == "ply":
            ...
            data_dict = []
            data_dict.append(
                {"X": float(self.points[:, 0]), "Y": float(self.points[:, 1]), "Z": float(self.points[:, 2])})
    ...
    def find_key_points(self, feature="fpfh", points="variable", quartile=0.6, num_points=100, dist_metric="cityblock"):
        ...
        def compute_distances_to_mu_histogram(self, metric="cityblock"):
            ...
            if metric == "cityblock":
                ...
                for i in range(self.fpfh.shape[0]):
                    dist[i] = scipy.stats.entropy(self.fpfh[i], mu_fpfh.flatten(),
                                                  base=None)  # Kulback-Leibler divergence
            else:
        ...
        if feature == "fpfh":
            ...
            if points == "variable":
                ...
                key_points_idx = \
                numpy.where((fpfh_distances < confid_interval[0]) | (fpfh_distances > confid_interval[1]))[0]
            elif points == "fixed":
    ...
    def find_persistent_key_points(self, feature="fpfh", dist_metric="cityblock", scales=[10, 5, 2], neighborhood=100,
                                   quartile=0.7):
        """
        Persistenc analysis:
        ...
        persistent_key_points = reduce(numpy.intersect1d,
                                       (key_sets[0], key_sets[1], key_sets[2]))  # TODO Make this flexible
    ...
    def estimate_normals_and_curvature_python(self, neighborhood, search_radius, compute_curvature=False):
        ...
        for idx, point in enumerate(self.points):
            ...
            nbrs = NearestNeighbors(n_neighbors=neighborhood + 1, algorithm="kd_tree", metric="euclidean",
                                    radius=search_radius).fit(self.points)
