...
# import kaolin
# import trimesh
def create_pcd_from_nparray(edge_arr):
    all_indices = np.array(np.nonzero(edge_arr > 0)).T
    pcd = create_pcd_from_coords(all_indices)
    return pcd
def create_pcd_from_coords(all_indices):
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(all_indices[:, :3])
    return pcd
...
if __name__ == '__main__':
    ...
    edge_draw = (edge_in_mask.cpu().round()).float()
    ...
    # pcd1 = pcd.uniform_down_sample(every_k_points=10)
    pcd.scale(1 / np.max(pcd_ori.get_max_bound() - pcd_ori.get_min_bound()),
              center=pcd_ori.get_center())
